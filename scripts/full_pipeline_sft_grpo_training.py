...
def run_sft_training(sft_config, sft_train_data_cot, sft_val_data, logger):
    ...
    if logger:
        ...
        logger.info(
            f"Using {len(sft_train_data_cot['prompts'])} high-quality CoT examples"
        )
    if logger:

        logger.info(
            f"Note: Checkpoints will be auto-saved every {sft_config['logging']['save_interval']} steps"
        )
    if logger:
    ...
    if logger:
        ...
        logger.info(
            f"Total time: {sft_results['total_time']:.2f} seconds ({sft_results['total_time']/60:.2f} minutes)"
        )
    if logger:
...
def visualize_grpo_results(
    grpo_training_metrics, grpo_validation_metrics, grpo, logger
):
    ...
    axes[0, 1].plot(
        grpo_training_metrics["episode"],
        grpo_training_metrics["reward_mean"],
        "purple",
        alpha=0.7,
        label="Mean",
    )
    axes[0, 1].fill_between(
        grpo_training_metrics["episode"],
        np.array(grpo_training_metrics["reward_mean"])
        - np.array(grpo_training_metrics["reward_std"]),
        np.array(grpo_training_metrics["reward_mean"])
        + np.array(grpo_training_metrics["reward_std"]),
        alpha=0.3,
        color="purple",
    ...
    axes[1, 0].plot(
        grpo_training_metrics["episode"],
        grpo_training_metrics["kl_divergence"],
        "r-",
        alpha=0.7,
    )
    ...
    if grpo_validation_metrics["episode"]:
        axes[1, 1].plot(
            grpo_validation_metrics["episode"],
            grpo_validation_metrics["exact_accuracy"],
            "go-",
            label="Exact Match",
            markersize=8,
        )
        axes[1, 1].plot(
            grpo_validation_metrics["episode"],
            grpo_validation_metrics["numeric_accuracy"],
            "bs-",
            label="Numeric Match",
            markersize=8,
        )
    ...
    if grpo_validation_metrics["episode"]:
        axes[2, 0].plot(
            grpo_validation_metrics["episode"],
            grpo_validation_metrics["format_compliance"],
            "mo-",
    ...
    axes[2, 1].plot(
        grpo_training_metrics["episode"],
        grpo_training_metrics["episode_time"],
        "orange",
        alpha=0.7,
        linewidth=2,
    )
    ...
    axes[3, 1].plot(
        grpo_training_metrics["episode"],
        grpo_training_metrics["tokens_per_second"],
        "darkblue",
        alpha=0.7,
        linewidth=2,
    )
    ...
    axes[4, 0].axhline(
        y=grpo.gradient_clip,
        color="r",
        linestyle="--",
        label=f"Clip={grpo.gradient_clip}",
        linewidth=1,
    )
    ...
    axes[4, 1].scatter(
        ...
        alpha=0.6,
        s=30,
        c=grpo_training_metrics["episode"],
        cmap="viridis",
    )
    ...
    if logger:
        ...
        logger.info(
            f"Final KL Divergence: {grpo_training_metrics['kl_divergence'][-1]:.4f}"
        )
    if logger:

        logger.info(
            f"Final Mean Reward: {grpo_training_metrics['reward_mean'][-1]:.3f}"
        )
    if logger:

        logger.info(
            f"Average Reward (last 5 episodes): {np.mean(grpo_training_metrics['reward_mean'][-5:]):.3f}"
        )
    if logger:

        logger.info(
            f"Total Tokens Processed: {grpo_training_metrics['total_tokens'][-1]:,}"
        )
    if logger:

        logger.info(
            f"Average Processing Speed: {np.mean(grpo_training_metrics['tokens_per_second']):.1f} tokens/second"
        )
    ...
    if logger:
        ...
        logger.info(
            f"Mean Gradient Norm: {np.mean(grpo_training_metrics['grad_norm']):.3f}"
        )
    if logger:

        logger.info(
            f"Max Gradient Norm: {np.max(grpo_training_metrics['grad_norm']):.3f}"
        )
    if logger:
...
def main():
    ...
    if device.type == "cuda":
        # Use BF16 on NVIDIA GPUs (better stability than FP16, less memory than FP32)
        model_dtype = "bf16"
        logger.info(f"Device: {device} → Using BF16 (bfloat16) for training")
    else:
        # Use FP32 on CPU/MPS for maximum stability
        model_dtype = "fp32"
        logger.info(f"Device: {device} → Using FP32 (float32) for training")
    ...
    cached_data = load_gsm8k_cache(
        cache_dir=DATASET_CACHE_DIR, splits=gsm8k_splits, system_prompt=SYSTEM_PROMPT
    )
    ...
    if cached_data is not None:
        ...
        grpo_val_prompts, grpo_val_answers = prepare_gsm8k_prompts(
            dataset_val, SYSTEM_PROMPT
        )
        grpo_test_prompts, grpo_test_answers = prepare_gsm8k_prompts(
            dataset_test, SYSTEM_PROMPT
        )
    ...
    if sft_train_data_cot is None:
        ...
        sft_train_data_cot = prepare_cot_dataset_for_sft(
            SYSTEM_PROMPT, num_examples=cot_num_examples, logger=logger
        )
        ...
        logger.info(
            f"✓ Loaded {len(sft_train_data_cot['prompts'])} CoT examples from cache"
        )
    ...
    logger.info(f"{'='*60}")
    logger.info(
        f"SFT Training samples: {len(sft_train_data_cot['prompts'])} (high-quality CoT)"
    )
    ...
    # --------------------------------------------------------
    if RUN_SFT:
        # SFT configuration with device-specific dtype
        # CUDA: BF16 (better stability than FP16, good memory efficiency)
        # CPU/MPS: FP32 (maximum stability)
        sft_config = {
            "model": {
    ...
    grpo_config = {
        ...
        "training": {
            ...
            "logprobs_batch_size": 16,  # Chunk logprob computation to reduce memory usage (None = no chunking)
    ...
    if CONTINUE_FROM > 0:
        ...
        grpo_checkpoint_path = Path(
            f"checkpoints/grpo_qwen_math/checkpoint_episode_{CONTINUE_FROM}.pt"
        )
        ...
        logger.info(
            f"  warmup_start_lr: {checkpoint_opt_config.get('warmup_start_lr')}"
        )
        logger.info("=" * 60)
        ...
        if config_lr > checkpoint_lr and lr_increase_ratio >= 1.5:
            ...
            resume_warmup_steps = grpo_config["optimizer"].get(
                "resume_warmup_steps", 15
            )
            logger.info(
                f"  LR increasing {lr_increase_ratio:.1f}x - applying {resume_warmup_steps}-step warmup: {checkpoint_lr:.2e} → {config_lr:.2e}"
            )
            ...
            if total_steps >= initial_warmup_steps:
                grpo.lr_scheduler = None
                logger.info(
                    f"  ✓ Scheduler disabled (past warmup), LR = {config_lr:.2e}"
                )
            else:
                logger.info(
                    f"  Still in warmup phase, advancing scheduler to step {total_steps}..."
                )
        ...
        if (
            "ref_policy_state_dict" in grpo_checkpoint
            and grpo_checkpoint["ref_policy_state_dict"] is not None
        ):
            ...
            logger.info(
                f"  ⚠️  WARNING: No reference policy in checkpoint - KL divergence will be incorrect!"
            )
    ...
    # 5. GRPO Training
    # --------------------------------------------------------
    grpo_train_data = {"prompts": grpo_train_prompts, "answers": grpo_train_answers}

    grpo_val_data = {"prompts": grpo_val_prompts, "answers": grpo_val_answers}
    ...
    logger.info("=" * 60)
    logger.info(
        f"Total time: {grpo_results['total_time']:.2f} seconds ({grpo_results['total_time']/60:.2f} minutes)"
    )
    logger.info(
        f"Total tokens processed: {grpo_results['training_metrics']['total_tokens'][-1]:,}"
    )
