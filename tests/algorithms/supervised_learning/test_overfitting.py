...
class TextClassificationModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(config.get("dropout", 0.1)), nn.Linear(hidden_size, num_classes)
        )
...
def overfitting_config():
    ...
    return dict(
        ...
        training={
            ...
            "learning_rate": 2e-5,
            "optimizer": {"type": "adamw", "betas": [0.9, 0.999]},
        },
        logging={"log_interval": 5, "eval_interval": 50},  # Less frequent eval
        wandb={"enabled": False},
    )
...
class TestOverfitting:
    ...
    def test_overfit_small_model_tiny_dataset(self, tiny_dataset, overfitting_config):
        ...
        ), f"Expected >80% accuracy, got {eval_results['accuracy']:.2%}"
        assert (
            eval_results["eval_loss"] < 0.5
        ), f"Expected low loss, got {eval_results['eval_loss']:.4f}"
        ...
        with torch.no_grad():
            for i, (text, true_label) in enumerate(zip(texts, labels)):
                ...
                device_encoding = {
                    k: v.to(algorithm.device) for k, v in encoding.items()
                }
    ...
    def test_overfit_with_frozen_backbone(self, tiny_dataset, overfitting_config):
        ...
        val_loader = DataLoader(
            train_dataset, batch_size=2, shuffle=False, collate_fn=collate_fn
        )
        ...
        ), f"Expected >60% accuracy with frozen backbone, got {eval_results['accuracy']:.2%}"
    ...
    def test_save_and_load_overfitted_model(self, tiny_dataset, overfitting_config):
        ...
        model_config = {"hf_model_name": "distilbert-base-uncased", "num_classes": 2}
        ...
        with tempfile.TemporaryDirectory() as tmp_path:
            ...
            with torch.no_grad():
                # Move encoding to device
                device_encoding = {
                    k: v.to(algorithm1.device) for k, v in encoding.items()
                }
                logits1 = model1(device_encoding)
                logits2 = model2(device_encoding)

            torch.testing.assert_close(
                logits1, logits2, msg="Loaded model gives different predictions"
            )
