...
class TestGRPOMasking:
    ...
    def config(self):
        ...
        return {
            ...
            "model": {"hf_model_name": "gpt2", "max_length": 128},
            "training": {
                "batch_size": 4,
                "learning_rate": 1e-5,
                "max_new_tokens": 10,
    ...
    def test_no_double_masking_in_compute_log_probs(self, config):
            ...
            received_masks.append(
                {
                    ...
                    "target_mask": (
                        target_mask.clone() if target_mask is not None else None
                    ),
                }
            )
        ...
        assert torch.all(
            target_mask[0, 5:15] == 1
        )  # Next 10 are completion (before padding)
        assert torch.all(target_mask[1, :8] == 0)  # First 8 are prompt
        assert torch.all(
            target_mask[1, 8:18] == 1
        )  # Next 10 are completion (before padding)
    ...
    def test_masking_with_variable_length_sequences(self, config):
        ...
        for i, (prompt_len, seq_len) in enumerate(zip(prompt_lengths, seq_lengths)):
            ...
            if completion_start < completion_end:
                ...
                assert torch.all(
                    actual_mask == expected_ones
                ), f"Seq {i}: completion mask failed"
    ...
    def test_policy_loss_receives_premasked_logprobs(self, config):
        ...
        log_probs[:, 5:10] = (
            torch.randn(batch_size, 5) * 0.1 - 2.0
        )  # Only completion tokens have values
    ...
    def test_kl_divergence_with_premasked_inputs(self, config):
        ...
        ref_log_probs[:, completion_range] = torch.tensor(
            [[-2.1, -2.0, -2.3, -2.2, -2.5], [-2.0, -1.9, -2.2, -2.1, -2.4]]
        )
